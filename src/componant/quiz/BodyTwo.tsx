...
//        text-black mt-3 font-light font-[Roboto] text-[16px]"
//       >
//         Step 2/5
//       </h1>
//       <h1
...
//       </h1>
//       <h1
//         className="text-[#6B665F] text-lg font-[Raleway]
//         flex justify-center items-center text-center mt-2
//          sm:text-[16px]
//         md:text-[18px]
...
//           <h1
//             className={`
//           flex justify-center font-[Raleway] mt-1 font-normal items-center text-[18px]
...
//              py-[13px] text-center cursor-pointer mt-24"
//         >
//           Continue
//         </button>
//       </div>
...
const BodyTwo: React.FC<BodyTwoProps> = ({ setPops, input, setInput }) => {
  ...
  const options = [
    { id: "option1", label: "Apartment" },
    { id: "option2", label: "Townhouse" },
    { id: "option3", label: "Villa" },
    { id: "option4", label: "Penthouse" },
  ];
  ...
  return (
    <div>
      ...
      >
        {options.map((option) => (
          <label
            key={option.id}
            htmlFor={option.id}
            className={`flex justify-start items-center w-[349px] h-[54px] text-black hover:text-[#CEA768] 
              border-2 border-[#C8C8CA] p-3 rounded-xl hover:border-[#CEA768] cursor-pointer
              ${
                input.step2 === option.id
                  ? "text-[#CEA768] border-[#CEA768]"
                  : ""
              }`}
          >
            <input
              ...
              type="radio"
              className="mr-5 w-[17px] h-[17px] cursor-pointer flex items-center justify-center 
              text-[#CEA768] appearance-none border-2 border-gray-400 hover:border-[#CEA768] 
              rounded-full checked:bg-[#CEA768] checked:border-transparent 
              focus:outline-3 focus:outline-offset-3 focus:outline-[#CEA768] active:bg-[#CEA768] ml-2"
              checked={input.step2 === option.id}
            />
            <h1
              className={`flex justify-center font-[Raleway] mt-1 font-normal items-center text-[18px] ${
                input.step2 === option.id ? "text-[#CEA768]" : ""
              }`}
            >
              {option.label}
            </h1>
      ...
      <div className="mt-13 font-semibold flex justify-center items-center lg:m-5 xl:mt-9 md:my-5 sm:my-5">
        <button
          onClick={three}
          className="hover:text-[#B78647] hover:bg-none hover:bg-white hover:border-2 hover:border-[#B78647] hover:duration-100
          text-white w-[25%] lg:w-auto bg-linear-to-r from-[#E2C385] to-[#B78647] text-[16px] 
